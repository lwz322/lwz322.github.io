...
---
在Microsoft Build 2019上微软发布了Windows Terminal，在2019年6月中旬，微软正式在应用商店上架了Windows Terminal的Preview版本，趁此机会配置一下Windows下的终端和shell，另外就是附带了对现阶段的Windows Terminal的配置文件，使其接近宣传图中的效果，
<!--more-->
...
另外在Scoop的dorado仓库中做了收录，添加仓库后安装即可（见后文）

### Json配置
在2019年6月中旬，微软正式在应用商店上架了Windows Terminal的Preview版本，配置文件部分变化比较大，现阶段只有Json的方式做配置，个人做了一些视觉方面的修改，下面是主要修改的部分：
```json
    "profiles" : 
    [
        {
...
```
这里要注意的一处，修改启动目录时（比如设定为wsl的~），如果使用```wsl -d debian```会无效，直接使用```debian```即可，参考自[Windows Terminal 的 WSL 配置](https://www.ixiqin.com/2019/08/windows-terminal-wsl-configuration/)
...
```json
{
            ...
            "fontFace" : "Consolas",
            "fontSize" : 11,
            "guid" : "{58ad8b0c-3ef8-5f4d-bc6f-13e4c00f2533}",
            "historySize" : 9001,
