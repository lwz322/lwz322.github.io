...
既然在外网使用的域名访问，还是开启HTTPS会相对安全（想象下未加密访问LuCI时，路由器的密码明文在网路上传输是有多危险），这里分别介绍：
...
还好我这里的移动宽带没有这个问题，不然的话只有把相应的服务改到其他的端口，或者使用端口转发，如群晖NAS默认HTTP端口就是5000，一般也是建议使用不常用的端口，高位端口好一点
