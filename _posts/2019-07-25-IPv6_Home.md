...
虽然说IPv6方便了点对点连接，但是普通家庭用户的IP地址（或者PD）依然是经常变化的，比如说重新拨号或者定期的被换IP，所以一个域名和DDNS还是需要的，好在实现成本不高，有免费的[dynv6](https://dynv6.com/)
...
需要注意的有
- 在Advance Setting选项卡中选择的IP address source [IPv6]选Interface为获得IPv6地址的接口，如果没有使用IPv6 NAT的话，选Interface为br-lan是相对方便的，一方面是只有一个地址（也就是LAN的IPv6的起始地址），不会泄露信息（对pppoe还好，mac一般是随机生成的），另外是方便看PD前缀
...
> 部分域名商如阿里云和DNSPod会提供域名的免费证书，想要省事的话可以去尝试
