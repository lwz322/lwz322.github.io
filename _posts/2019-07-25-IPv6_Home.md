...
在OpenWrt的LuCi界面的Network->Firewall->Traffic Rules一栏可以看到各种防火墙的规则，可以看到OpenWrt是默认允许外网Ping内网的（Allow-ICMPv6-Forward）
...
既然在外网使用的域名访问，还是开启HTTPS会相对安全，这里分别介绍Let's encrypt的单一域名证书和通配符证书申请的方法

### 单一域名证书申请

单一证书签发参考了[LEDE申请并部署Let's encrypt证书的教程](https://koolshare.cn/thread-126778-1-1.html)，使用的是[Neilpang/acme.sh](https://github.com/Neilpang/acme.sh/wiki/)的脚本和阿里云的域名，使用acme.sh实现了acme协议, 可以从letsencrypt生成免费的证书并保持自动更新
> 本文申请的证书有效期都是三个月，使用acme.sh的情况下会自动注册一个续期的计划任务

acme的官方教程也可以看[How to run on OpenWRT](https://github.com/Neilpang/acme.sh/wiki/How-to-run-on-OpenWRT)，也有人开发了luci-app-acme，但是我没有设置成功...网上例子几乎找不到
...
OpenWrt 18.06的uhttpd已经默认开启了443端口的监听，不需要再做设置（不放心就检查下吧）
...
但是需要注意的是使用IP直接进行访问时，依然会提示不安全，因为一般的证书是和域名绑定的，其实内网也完全可以使用域名进行访问（在有网络的前提下），或者添加本地DNS解析记录（修改OpenWrt路由器的Hosts:```/etc/host```）

然而某些情况下（如局域网的二级路由下），OpenWrt路由下的设备不能解析指向内网IP段的域名，在Network->DHCP and DNS页面内的Rebind protection是默认开启的，其作用在于避免本地设备受到DNS相关的攻击，故路由器会将**上游路由**的对RFC1918域（0.0.0.0/8, 172.16.0.0/12 and 192.168.0.0/16，即内网IP段）的响应包扔掉
...
对于群晖的NAS，在证书页面添加私钥和证书的pem文件，再开启HTTPS的5001端口访问即可

如果把证书安装到OpenWrt的路由器中，会遇到一个问题：uhttpd默认只支持der格式的证书，此处需要转换证书的格式，这里使用openssl：
```shell
