...
# 调试代码
VSCode全称Visual Studio Code，调试代码方面算是对得起Visual Studio之名了，而调试代码的配置逻辑上有点意思，这里就记录下在Windows下的配置经过，仅供参考
...
到了VSCode当然会想要接近Visual Studio的体验，编译器肯定不用Clang了，至于GCC，WSL里有，Win上的GCC的版本不知道用哪个好，看网上的博客还是有些麻烦，使用WSL GCC可以参考下面的链接（和上个链接的WSL Remote还是有些不同的）：
[VSCode使用WSL环境开发C语言配置](https://www.nomox.cn/post/develop-c-within-wsl-by-vscode/)

直到后面遇到了[Scoop]()，安装就很简单了：```scoop install gcc```，查看版本后发现是MinGW的GCC，安装CodeRunner扩张后就可以运行代码了
...
- 以上的文件在做了选择之后就自动生成好了，之后改文件夹内的C代码都可以透过VSCode的Debug来调试了，设置断点，查看变量体验还是比较现代的~
