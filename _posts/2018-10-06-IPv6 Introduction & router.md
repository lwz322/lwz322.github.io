...
[蒲公英PT](https://npupt.com/)，西北工业大学的PT站，也是支持部分IPv6的校外登陆，个人觉得使用体验在教育网PT里面是最好的
...
用手机4G作为IPv6接入，使用OpenWrt路由器无线中继或者USB共享手机的网络，之后再设置IPv6 Relay，核心是在连接收集的接口上建立一个DHCPv6 Client（即下面的wwan_6），之后设IPv6的中继模式
```
# cat /etc/config/dhcp
config dhcp wwan_6
    option dhcpv6 relay
