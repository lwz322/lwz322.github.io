...
---
查询宿舍电费余额的爬虫，因为用requests库怎么也登录不上去，所以就用了selenium库
<!--more-->
---
# 电费查询脚本
...
如果是linux平台的话，在纯命令行的情况下（无GUI，如WSL），可能需要安装```xvfb```为虚拟GUI，然后再在python中安装```pyvirtualdisplay```


WSL的话可以再Win10下安装并开启VcXsrv作为代替
...
这里使用crontab来添加计划任务，比如说每隔一个小时查询并记录一次,并且把错误重定向到正常的输出中
...
```shell
#!/bin/sh
python3PYTHON_SCRIPT_PATH >> YOUR_LOG_PATH
```
至于具体的输出格式就看个人的定制了
