...
一个支持实时渲染的，跨平台的Markdown编辑器，平时拿来写点公式特别方便

[官网](https://typora.io/)

设置建议使用把md文件引用的图片统一保存到本地，方便备份和跨设备，也可以使用图床（说不定哪天就没了）

## PicGo
[Molunerfinn/PicGo](https://github.com/Molunerfinn/PicGo)
又是一个相见恨晚的软件，支持从剪贴板复制图片，自动上传并输出Markdown引用URL的格式
...
也可以根据官方的API文档按照喜好写，自己就写了一个简易的版本：
![](https://raw.githubusercontent.com/lwz322/pics/master/mathgo.gif?token=AFCJDWPPZ4TGV2AFVNVI3G254FQM4)
...
美中不足的，可能因为连不上服务器而不能用,远程IP都是AWS的服务器（最近怎么样就不清楚了）
...
因为周围的同学主要还是在Windows环境下，平时最多也只是用下WSL(windows的Linux子系统)，所以就简单的移植了下，效果如图：

![](https://img.vim-cn.com/51/1f2d50974263e4ef0882c44a5cde51fc5d0138.png)
