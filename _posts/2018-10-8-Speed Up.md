...
设计要达到的目标，用选项控制配置的添加和删除

方便设置PPPoE的账号，密码

这段也可以添加到ifup的hotlpug文件中
...
do
  ...
  uci set network.VWAN$i.username="jcy2"
  uci set network.VWAN$i.password="131318"
...
这里为了避免原firewall zone中firewall.@zone[1].network='wan wan6 VWAN2 wwan' 不好用uci add_list添加更多的接口到网络中，所以选择自己再添加一个firewall zone
...
#这段也可以添加到ifup的hotlpug文件中，暂时对hotplug不了解
...
do
  ...
  uci set mwan3.VWAN$i.track_ip='10.170.72.254'
...
从上面来看，IPv6的路由表应该是原生就支持负载均衡的（weight）
...
利用的是NAT之后，把内网的IP地址添加到防火墙规则当中（这种方法也适合后期添加接口），如果已经在NAT6配置过程中配置好了的话这一步就可以忽略了
...
目前最新版本的mwan3也开始支持IPv6多拨了，但是个人还是觉得日常使用的卷话用以上的方法更加快捷

实测在K2P不开启HWNAT的情况下，IPv6四线八拨的速度最高在300Mbps左右，开启HWNAT就有400Mbps了，也就到达了在学校宿舍四个百兆端口的情况下的最大带宽

因为对路由表的修改在路由器重启之后会重置，所以还是要添加到开机的启动脚本之中
...
首先就是设置VLAN，这里就不细说了，每个人的路由器情况不同
