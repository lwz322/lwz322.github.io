...
主要详细介绍了OpenWrt内网配置IPv6的方法，回想起当年的配置经历，只能说有些幸运吧，因为处理的各种问题比较多，这里写的相比于网络上一般的配置文章要详细得多
...
iface_route=pppoe-wan
```
这是针对一般的默认情况写的，也就是名为wan的口以PPPoE连接网络，然后在系统中显示的接口名就变成了pppoe-wan，一般默认设置OpenWrt对PPPoE使用了内建的IPv6管理，所以会虚拟出一个IPv6接口（DHCPv6客户端，不可编辑，能通过命令查到），其接口名是原名称后面接上_6，也就是wan_6，如果是eth0.2直接获取的IPv6地址，那么这个接口就是wan6了
