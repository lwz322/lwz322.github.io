...
介绍了OpenWrt使用NAT配置IPv6的方法，回想起当年的配置经历，只能说有些幸运吧，因为处理的各种问题比较多，这里写的相比于网络上一般的配置文章要详细得多

<!--more-->
在之前的一文中已经阐述了IPv6的基本情况，在大多数情况下IPv6 NAT还是适用的，主要介绍设置步骤以及背后的一些原理
...
连接之后就可以获取IPv6地址（可能要等一段时间），如果是如下这样的本地链路地址
...
在官网的中的方法 则被描述为：The solution provided here can be considered more robust and portable，只是粗浅的看了下，大概是通过添加解析配置文件来实现的，真厉害，不过个人还是偏向于本文的方法，简单而模块化的设计方便做进一步的设置
