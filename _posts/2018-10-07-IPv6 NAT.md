...
这一条网关是把经过路由器获得的前缀网段的数据包发送到上级交换机的端口

>  比如说路由器``ping6``的时候，发送的是来自路由器本身的数据包，路由器做全局IPv6路由的时候，下级设备的前缀与路由器相同，数据包直接被发送到上级交换机

而实际上来自下级设备的数据包发送的时候的流程是

![iptables](C:\Users\lwz32\Documents\GitHub\lwz322.github.io\_posts\assets\3652bb519e2f632b0a459461d2779a588e92b4.svg)
...
其中提到了需要修改ULA前缀第一个字母为d（也就是变成了表格第一行的保留地址段），不然客户端可能会**优先采用 IPv4**，为了避免这种情况（主要是**校园网流量有限**），只好修改一下ULA ，因为经过了NAT6，最终发出的数据包的来源地址都是路由器的地址，应该是不会有冲突的
...
**注**：Hotplug功能实际上是相当的实用的，https://github.com/wywincl/hotplug这篇文章做了详细的剖析
...
在IPv6路由表中添加条目，去掉原路由表中的from 2001:xxx:xxx:xxx/64字段，设置上级交换机端口为默认网关
...
添加NAT6到防火墙规则中，这里采用的是在数据包路由之后对来自ULA地址前缀的包做SNAT——替换源地址为路由器本身的全局IPv6地址
...
可以在通过添加路由器的hosts文件的方法让IPv6网络更加畅通

https://github.com/lennylxx/ipv6-hosts

命名为hosts6，再使用额外添加的方法就好
