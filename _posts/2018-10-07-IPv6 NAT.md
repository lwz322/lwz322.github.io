...
**注**：此脚本适用于较新的OpenWRT，并且在条件与 **我的设备** 差不多才行，如果配置失败，建议重置设置，之后参考下面的原理部分自行修改测试
...
IPv6设计的初衷之一就是消除NAT，所以直到比较晚的时候才在添加了IPv6 NAT的RFC，并且也不建议使用，与之相关的工具并不普及，比如说IPv6 NAT需要Linux Kernel version大于3.7
...
iface_route=pppoe-wan
```
这是针对一般的默认情况写的，也就是名为wan的口以PPPoE连接网络，然后在系统中显示的接口名就变成了pppoe-wan，一般默认设置OpenWrt对PPPoE使用了内建的IPv6g管理，所以会虚拟出一个IPv6接口（DHCPv6客户端，并且是在不可编辑的，能通过命令查到），其接口名是原本的名称后面接上_6，也就是wan_6，这里对接口单独设置了变量，可以按照实际情况修改，比如说，如果是指定DHCPv6客户端模式的接口是IPv6接入，把第一个接口名改为获取IPv6网络的接口，第二个改为指向上级网关的接口

**注**：这里是以wan_6的插拔来侦测IPv6连接，而添加的网关是通过iface_route来获取的
> **OpenWRT的接口**
