...
至于为什么要懂一点点原理，因为本文中所写的方法在学校是最简单的，然而实际中遇到什么情况都有可能，所以也就没有什么最好的方法了，这点可以参考第二个参考链接里的 2018 Version，原理依旧适用，只是实现方法和侧重点略有不同，而在官网的中的方法 [NAT6: IPv6 Masquerading Router](https://openwrt.org/docs/guide-user/network/ipv6/ipv6.nat6)则被描述为：The solution provided here can be considered more robust and portable，下文给出的方法参考了网络上的各种文章并且细查了原理，实际用了很长一段时间被证明是稳定可行的
...
```
>LEDE曾经从OpenWRT项目分离出来，后来又合并回去了，然而科大的源还是注明的lede，里面有OpenWRT的源就是了
...
iface_linux=pppoe-wan
```
这是针对一般的默认情况写的，也就是名为wan的口以PPPoE连接，然后在系统中显示的接口名就变成了pppoe-wan，因为OpenWrt对PPPoE会虚拟出一个IPv6接口（DHCPv6客户端，并且是不可编辑的），其LuCi上的名字是原本的名称后面接上_6，也就是wan_6，这里对接口单独设置了变量，可以按照实际情况修改，比如说，如果是指定DHCPv6客户端模式的接口是IPv6接入，把两个变量都改成这个接口名就好
**注**：这里是以wan_6的插拔来侦测IPv6连接，而添加的网关是通过iface_linux来获取的
