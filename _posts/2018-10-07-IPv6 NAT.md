...
**注**：此脚本适用于较新的18.06.1，因为PPPoE虚拟的IPv6接口改名为```wan_6```，如果是其他的版本，需要自行查看接口名称判断，而其他可能会有变化的部分会在下文给出，并且在条件与 **我的设备** 高度吻合才行，如果情况有些不一样的话可以参考下面的原理部分自行修改测试
...
**注**：貌似18.06安装nat6模块之后会添加该默认网关条目，不过多写一步也没坏处
