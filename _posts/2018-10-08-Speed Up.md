...
不需要关心IP，只要是不同的连接就可以做负载均衡，典型的如使用多线程下载软件下载一个文件，本文主要做到的都是这一步，尽管没有做到Per-Packet那么极致，但是稍微加速下日常的应用还是有帮助的，比如Chrome对单个域名可以并发的请求是六个，也就有可能六个连接会被分到多个出口上，进而起到一定的加速效果，多线程下载同理，连接数越高，利用率就越高
