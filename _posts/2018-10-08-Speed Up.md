...
400Mbps网速成就达成！主要介绍单线多拨，多线多拨以及一种简单负载均衡配置方法（ECMP，等效多径路由），也为这些配置方法写了脚本，节约配置的时间，特别的，教育网NAT6下IPv6多拨使用ECMP可以有非常好的效果


<!--more-->

## 前言
什么是多拨？简单的顾名思义，对最常见的PPPoE拨号接入，就是在一台设备上拨多个号，然而仅仅是拨多个号还是不够的，查看路由表可以发现，多条PPPoE路由是有使用的先后次序的（metric）

## 已有的解决方案
配置繁琐的这个坑还是有人填的，包括mwan3的IPv6相关的问题以及负载均衡都有人写了patch:

首先是lean的lede中的插件：[lean/luci-app-syncdial](https://github.com/coolsnowwolf/lede/tree/master/package/lean/luci-app-syncdial)

之后有人添加了IPv6负载均衡的补丁，放出了修改后的源码: [多播插件添加ipv6负载均衡](https://www.right.com.cn/forum/forum.php?mod=viewthread&tid=760177)

我编译了下，看起来还不错，配置起来算是非常友好了，然而本文适合更加极限一点的情况（拨号的数量更多），以及对IPv6负载均衡没有使用mwan3（我还是关掉了mwan3的服务）
![](https://i.loli.net/2019/10/30/nCiRvgeoQNB8PMw.png)


## 单线多拨

教程或者现成的东西网络上实在是太多了，大概翻阅了一下，缺的大概是脚本和细节上的说明吧（GUI实在是太繁琐了，费时费力），毕竟在学校，难免会有一些想法
...
下面这段只在K2P OpenWrt 18.06上面实践过，而LEDE17.01无效，可能的原因是IPv6负载均衡是MWAN3更新之后添加的功能，在学校个人还是觉得IPv6多拨最实用，因为IPv6不计流量，多数情况下是用来挂机下载的，不用考虑延迟，也更加容易探到链路带宽的天花板，但是配置mwan软件的负载均衡又相对麻烦，这里又一种偶然发现的简单的方法

> 之后查看了mwan3的Git commit记录发现，早在17.01软件仓库的mwan3 2.02就已经支持了IPv6（影响上可能是luci-app-mwan3）
